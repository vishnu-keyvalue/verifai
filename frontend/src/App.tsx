...
const VerdictIcon = ({ verdict }: { verdict: string }) => {
  ...
  return (
    <div className={`w-16 h-16 ${style.bgColor} rounded-full flex items-center justify-center ring-4 ${style.ringColor} shadow-lg`}>
...
const StatsCard = ({ icon: Icon, label, value, color = "blue" }: {
  ...
  return (
    <div className={`p-4 rounded-xl border-2 ${colorClasses[color as keyof typeof colorClasses]} transition-all duration-200 hover:shadow-md`}>
...
const ConfidenceMeter = ({ confidence, label }: { confidence: number; label: string }) => {
    ...
    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
      ...
      <div className="flex items-center gap-2">
        <div className="w-20 bg-gray-200 rounded-full h-2">
          <div 
            className={`h-2 rounded-full transition-all duration-500 ${confidence >= 80 ? 'bg-green-500' : confidence >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}
            style={{ width: `${confidence}%` }}
          />
        </div>
        <span className={`text-sm font-bold px-2 py-1 rounded ${getColor(confidence)}`}>
          {confidence}%
        </span>
      </div>
    </div>
...
export default function App() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (data.verdict_analysis || data.verdict) {
  ...
  const renderVerdictTitle = () => {
    ...
    if (factCheckResult) {
      return factCheckResult.verdict_analysis?.verdict || factCheckResult.verdict || "Analysis Complete";
    }
            ...
            <div className="animate-fade-in">
              ...
              <div className="text-center mb-12">
                ...
                <p className="text-xl text-gray-600 mb-8 max-w-3xl mx-auto leading-relaxed">
                  Analyze images, links, and text to detect AI generation and verify factual claims with advanced machine learning algorithms.
                </p>
                  ...
                  {(["upload", "link", "text"] as ActiveTab[]).map((tab) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {tab === "upload" ? "Upload File" : tab === "link" ? "Paste Link" : "Paste Text"}
                      </button>
                    );
                  })}
                </div>
                ...
                {activeTab === "link" && (
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-gray-500 flex items-center gap-2">
                      <Info className="w-4 h-4" />
                      We'll analyze the content and verify claims from news articles, blog posts, and web pages
                    </p>
                  </div>
                )}
                      ...
                      {(analysisDetails || factCheckResult) && (
                        <div className="flex items-center gap-4 text-sm text-gray-600">
                          <div className="flex items-center gap-1">
                            <Clock className="w-4 h-4" />
                            {factCheckResult?.processing_metadata?.processing_time_seconds 
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {analysisDetails 
                              ...
                              ? `${Math.round(factCheckResult.verdict_analysis.confidence_score * 100)}% confidence`
                              : "Analysis complete"
                            }
                          </div>
                        </div>
                    ...
                    <div className="space-y-6">
                      <div className="prose max-w-none">
                        <p className="text-gray-700 text-lg leading-relaxed">
                          Our advanced AI model analyzed the image for digital artifacts, inconsistencies, and patterns commonly found in AI-generated content.
                        </p>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-gray-50 p-6 rounded-xl">
                          ...
                          <p className="text-gray-700 leading-relaxed">
                            Identified as <span className="font-bold text-purple-600">
                              {analysisDetails.modelPrediction.prediction}
                            </span> with <span className="font-bold">
                  ...
                  {factCheckResult && (
                    <div className="space-y-8">
                      ...
                      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-200">
                        <h3 className="font-semibold text-lg mb-3 text-blue-800">Primary Claim</h3>
                        <blockquote className="text-gray-700 italic text-lg leading-relaxed border-l-4 border-blue-400 pl-4">
                          "{factCheckResult.verdict_analysis?.primary_claim || factCheckResult.claim || "Analysis completed"}"
                        </blockquote>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                          <ConfidenceMeter 
                            confidence={Math.round((factCheckResult.verdict_analysis?.confidence_score || 0) * 100)} 
                            label="Overall Confidence" 
                          />
                          {factCheckResult.verdict_analysis?.credibility_breakdown && (
                            <>
                              <ConfidenceMeter 
                                confidence={Math.round(factCheckResult.verdict_analysis.credibility_breakdown.evidence_consistency * 100)} 
                                label="Evidence Consistency" 
                              />
                              <ConfidenceMeter 
                                confidence={Math.round(factCheckResult.verdict_analysis.credibility_breakdown.source_credibility_average * 100)} 
                                label="Source Credibility" 
                              />
                              <ConfidenceMeter 
                                confidence={Math.round(factCheckResult.verdict_analysis.credibility_breakdown.source_diversity * 100)} 
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <StatsCard 
                              icon={Shield} 
                              label="Fact Checkers" 
                              value={factCheckResult.evidence_analysis.sources_by_type?.fact_check || 0}
                              color="blue"
                            />
                            <StatsCard 
                              icon={Award} 
                              label="Academic" 
                              value={factCheckResult.evidence_analysis.sources_by_type?.academic || 0}
                              color="green"
                            />
                            <StatsCard 
                              icon={Globe} 
                              label="News Sources" 
                              value={factCheckResult.evidence_analysis.sources_by_type?.news || 0}
                              color="purple"
                            />
                            <StatsCard 
                              icon={CheckCircle} 
                              label="Official" 
                              value={factCheckResult.evidence_analysis.sources_by_type?.official || 0}
                              color="orange"
                            />
                          </div>
                          <div className="bg-gray-50 p-4 rounded-lg">
                            <div className="flex justify-between items-center text-sm">
                              <span className="text-gray-600">Total sources analyzed:</span>
                              <span className="font-bold">{factCheckResult.evidence_analysis.total_sources}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm mt-1">
                              <span className="text-gray-600">Average credibility:</span>
                              <span className="font-bold">{Math.round(factCheckResult.evidence_analysis.average_credibility * 100)}%</span>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {/* Risk Factors */}
                      {factCheckResult.verdict_analysis?.risk_factors && factCheckResult.verdict_analysis.risk_factors.length > 0 && (
                        <div className="bg-red-50 border border-red-200 p-6 rounded-xl">
                          ...
                          <ul className="space-y-2">
                            {factCheckResult.verdict_analysis.risk_factors.map((factor: string, index: number) => (
                      ...
                      {factCheckResult.verdict_analysis?.verification_suggestions && factCheckResult.verdict_analysis.verification_suggestions.length > 0 && (
                        <div className="bg-blue-50 border border-blue-200 p-6 rounded-xl">
                          ...
                          <ul className="space-y-2">
                            {factCheckResult.verdict_analysis.verification_suggestions.map((suggestion: string, index: number) => (
                              <li key={index} className="flex items-start gap-2">
